...
/* ─────────────────────────────────────────────────────────────────
   CUSTOM MUSIC SYSTEM  v2
   ─────────────────────────────────────────────────────────────────
   Players pick a folder of audio files (.ogg/.mp3/.wav/.flac).
   The game reads the filenames, shows a track list, and the player
   chooses which track plays during menus and/or battles.
   Works in Electron (full FS access) and modern browsers
   (File System Access API with fallback to <input type=file>).

   Tracks are stored as File objects in memory — they are never
   uploaded anywhere. The folder path is never saved (browser
   security). On Electron, the path CAN be saved via
   window.electronAPI.getFilePath(file).
───────────────────────────────────────────────────────────────── */

let _customMusicFiles   = [];   // Array of File objects from the chosen folder
let _customMenuTrack    = null; // File selected for menu music
let _customBattleTrack  = null; // File selected for battle music
let _customAudioEl      = null;
let _customAudioObjectUrl = null; // revokeable object URL for current track
...
/* ── Play a File object ── */
function _playCustomFile(file) {
    if (!file || muted) return;
    // Revoke previous object URL to free memory
    if (_customAudioObjectUrl) {
        URL.revokeObjectURL(_customAudioObjectUrl);
        _customAudioObjectUrl = null;
    }
    _customAudioObjectUrl = URL.createObjectURL(file);
    const el = _getOrCreateAudioEl();
    el.src    = _customAudioObjectUrl;
    el.volume = musicVol;
    el.loop   = true;
    el.play().catch(e => console.warn('[CustomAudio] play failed:', e));
}

/* ── Start custom audio (called from startMenuAudio / startBgAudio) ── */
function startCustomAudio(isBattle = false) {
    const track = isBattle
        ? (_customBattleTrack || _customMenuTrack)
        : _customMenuTrack;
    if (!track) return false; // no custom track — let caller use generated music
    _playCustomFile(track);
    return true;
}

function stopCustomAudio(fade = false) {
    if (!_customAudioEl) return;
    if (fade) {
        const el = _customAudioEl;
        const step = () => {
            el.volume = Math.max(0, el.volume - 0.05);
            if (el.volume > 0) setTimeout(step, 40);
            else { el.pause(); el.volume = musicVol; }
        };
        step();
    } else {
...
async function openMusicFolder() {
    ...
    const list   = document.getElementById('custom-music-track-list');

    // Try File System Access API first (Chrome 86+, Electron)
    if (window.showDirectoryPicker) {
        try {
            const dir = await window.showDirectoryPicker({ mode: 'read' });
            _customMusicFiles = [];
            for await (const entry of dir.values()) {
                if (entry.kind === 'file' && /\.(mp3|ogg|wav|flac|aac|m4a)$/i.test(entry.name)) {
                    const file = await entry.getFile();
                    _customMusicFiles.push(file);
                }
            }
            _customMusicFiles.sort((a,b) => a.name.localeCompare(b.name));
            _renderTrackList(list, status);
            return;
        } catch(e) {
    ...
    // Fallback: hidden <input type=file multiple> for older browsers
    let input = document.getElementById('_music-folder-input');
    if (!input) {
        input = document.createElement('input');
        input.id       = '_music-folder-input';
        input.type     = 'file';
        input.multiple = true;
        input.accept   = '.mp3,.ogg,.wav,.flac,.aac,.m4a';
        input.style.display = 'none';
        document.body.appendChild(input);
        input.addEventListener('change', () => {
            _customMusicFiles = Array.from(input.files)
                .filter(f => /\.(mp3|ogg|wav|flac|aac|m4a)$/i.test(f.name))
...
/* ── Render track list in settings ── */
function _renderTrackList(listEl, statusEl) {
    if (!listEl) return;
    if (!_customMusicFiles.length) {
        listEl.innerHTML = '<div style="font-family:Cinzel,serif;font-size:9px;color:#5a3a10;padding:8px 0;">No audio files found in that folder.</div>';
        return;
    }

    if (statusEl) {
        statusEl.textContent = `${_customMusicFiles.length} track${_customMusicFiles.length !== 1 ? 's' : ''} loaded`;
        statusEl.style.color = '#6b9a40';
    }

    listEl.innerHTML = _customMusicFiles.map((f, i) => {
        const isMenu   = _customMenuTrack?.name   === f.name;
        const isBattle = _customBattleTrack?.name === f.name;
        return `
        <div class="music-track-row" data-idx="${i}">
            <span class="music-track-name">${f.name.replace(/\.[^.]+$/, '')}</span>
            <div class="music-track-btns">
                <button class="music-track-btn ${isMenu ? 'active' : ''}"
                    onclick="setCustomTrack(${i}, 'menu')"
                    title="Use for menus">🎵 Menu</button>
                <button class="music-track-btn ${isBattle ? 'active' : ''}"
                    onclick="setCustomTrack(${i}, 'battle')"
                    title="Use for battles">⚔️ Battle</button>
                <button class="music-track-btn"
                    onclick="previewCustomTrack(${i})"
...
function setCustomTrack(idx, slot) {
    const file = _customMusicFiles[idx];
    if (!file) return;
    if (slot === 'menu')   _customMenuTrack   = file;
    if (slot === 'battle') _customBattleTrack = file;
    const list   = document.getElementById('custom-music-track-list');
    const status = document.getElementById('custom-music-status');
    _renderTrackList(list, status);
    if (status) {
        status.textContent = `${slot === 'menu' ? 'Menu' : 'Battle'} track set to: ${file.name.replace(/\.[^.]+$/, '')}`;
        status.style.color = '#6b9a40';
    }
    saveSettings();
}

function previewCustomTrack(idx) {
    const file = _customMusicFiles[idx];
    if (!file) return;
    stopCustomAudio(false);
    stopMenuAudio();
    stopBgAudio();
    _playCustomFile(file);
}
...
/* Dispatcher: picks which menu music track to start based on the user's
   saved preference (#opt-menu-track). The individual startMenuAudioTrackN()
   functions all existed and worked fine — this dispatcher was the missing
   piece connecting the setting to any of them, so menu music silently
   never played at all (the call was wrapped in a try/catch in
   _bootMenuMusic, which swallowed the "not defined" error with no trace). */
function startMenuAudio() {
    const sel = document.getElementById('opt-menu-track');
    const val = sel ? sel.value : '0';
    try {
        if (val === '99' && typeof playCustomMenuTrack === 'function' && window._customMusicFiles?.menu) {
            playCustomMenuTrack();
        } else if (val === '2') {
            startMenuAudioTrack3();
        } else if (val === '1') {
            startMenuAudioTrack2();
        } else {
            startMenuAudioTrack1(); // covers '0' (default) and any unrecognized value
        }
    } catch (e) {
        console.warn('[music] menu track failed to start, falling back to default', e);
        try { startMenuAudioTrack1(); } catch (e2) {}
    }
}

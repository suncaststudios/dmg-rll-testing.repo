...
const _matrixChars = 'アカサタナハマヤラワ0123456789ハミラ日ロミグウニ'.split('');
let _matrixCols = [];
function _initMatrixRain() {
    const colW = 16;
    const count = Math.ceil(emberCanvas.width / colW);
    _matrixCols = Array.from({length: count}, () => Math.random() * -100);
}
function animMatrixRain() {
    const colW = 16;
    if (_matrixCols.length !== Math.ceil(emberCanvas.width / colW)) _initMatrixRain();
    eCtx.fillStyle = 'rgba(5,0,10,0.18)'; // trailing fade instead of hard clear — gives the streak look
    eCtx.fillRect(0, 0, emberCanvas.width, emberCanvas.height);
    eCtx.font = '14px monospace';
    const speed = _graphicsQuality === 'low' ? 0 : (_graphicsQuality === 'high' ? 1.4 : 1);
    _matrixCols.forEach((y, i) => {
        if (speed === 0) return;
        const ch = _matrixChars[Math.floor(Math.random() * _matrixChars.length)];
        eCtx.fillStyle = Math.random() < 0.05 ? 'rgba(255,0,220,0.9)' : 'rgba(255,0,180,0.55)'; // magenta, occasional bright flash
        eCtx.fillText(ch, i * colW, y);
        _matrixCols[i] += 16 * speed;
        if (_matrixCols[i] > emberCanvas.height && Math.random() > 0.975) _matrixCols[i] = Math.random() * -100;
    });
}

/* ── Falling bullets + magical smoke (Casting Casings theme) ── */
const _bullets = [], _smokePuffs = [];
function _spawnBullet() {
    return {
        x: Math.random() * emberCanvas.width, y: -10,
        dy: Math.random() * 1.2 + 0.6, len: Math.random() * 10 + 6,
        life: 1, hue: 35 + Math.random() * 15, // brass/bronze
    };
}
function _spawnSmoke() {
    return {
        x: Math.random() * emberCanvas.width, y: emberCanvas.height + 20,
        r: Math.random() * 30 + 20, dx: (Math.random() - 0.5) * 0.3, dy: -(Math.random() * 0.4 + 0.15),
        life: 1, decay: Math.random() * 0.0015 + 0.0006,
    };
}
function animCastingCasings() {
    eCtx.clearRect(0, 0, emberCanvas.width, emberCanvas.height);
    const targetBullets = _graphicsQuality === 'low' ? 0 : _graphicsQuality === 'high' ? 26 : 14;
    const targetSmoke   = _graphicsQuality === 'low' ? 0 : _graphicsQuality === 'high' ? 10 : 5;
    while (_bullets.length < targetBullets) _bullets.push(_spawnBullet());
    while (_bullets.length > targetBullets) _bullets.pop();
    while (_smokePuffs.length < targetSmoke) _smokePuffs.push(_spawnSmoke());
    while (_smokePuffs.length > targetSmoke) _smokePuffs.pop();

    // smoke (drawn first, behind bullets)
    _smokePuffs.forEach((s, idx) => {
        s.x += s.dx; s.y += s.dy; s.life -= s.decay;
        if (s.life <= 0) { _smokePuffs[idx] = _spawnSmoke(); return; }
        const g = eCtx.createRadialGradient(s.x, s.y, 0, s.x, s.y, s.r);
        g.addColorStop(0, `rgba(180,140,220,${s.life * 0.10})`);
        g.addColorStop(1, 'transparent');
        eCtx.beginPath(); eCtx.arc(s.x, s.y, s.r, 0, Math.PI * 2);
        eCtx.fillStyle = g; eCtx.fill();
    });
    // bullets — thin falling brass streaks
    _bullets.forEach((b, idx) => {
        b.y += b.dy; b.life -= 0.0018;
        if (b.life <= 0 || b.y > emberCanvas.height + 20) { _bullets[idx] = _spawnBullet(); return; }
        eCtx.strokeStyle = `hsla(${b.hue}, 70%, 60%, ${b.life * 0.6})`;
        eCtx.lineWidth = 1.4;
        eCtx.beginPath(); eCtx.moveTo(b.x, b.y); eCtx.lineTo(b.x, b.y - b.len); eCtx.stroke();
    });
}

function animEmbers() {
    // Themes with a genuinely distinct background effect (not just a
    // color filter on the same ember particles) branch off here.
    const bodyClass = document.body.className;
    if (bodyClass.includes('theme-cyberpunk'))      { animMatrixRain();     requestAnimationFrame(animEmbers); return; }
    if (bodyClass.includes('theme-castingcasings'))  { animCastingCasings(); requestAnimationFrame(animEmbers); return; }
...
/* ── Music volume slider — the actual synth SFX engine (AC, muted, sfxVol,
   getAC, osc/noise, THEME_SFX, the full SFX library, and playSfx) has
   moved to sounds/sfx code/sfx-engine.js, loaded just before this file so
   AC/muted/sfxVol/getAC/playSfx are all still available here and in every
   file after this one via shared top-level scope, exactly as before. ── */
...
// startBgAudio / stopBgAudio / startMenuAudio / stopMenuAudio now live in
// music.js (the real per-theme .ogg/.mp3/.opus player).
...
function toggle(id, show) {
    ...
    if (show) {
        // menu-main uses a CSS grid two-column layout (#menu-main { display:
        // grid; ... } in main.css). Force-setting inline display:flex here —
        // which used to happen for every element including menu-main itself
        // — permanently overrode that grid rule (inline styles always beat
        // stylesheet rules) the first time this ran, collapsing the left
        // panel/showcase layout into one stacked column. Clear any leftover
        // inline override on menu-main instead, so the CSS grid rule applies
        // as intended; submenus still explicitly need flex.
        el.style.display = isSubMenu ? 'flex' : '';
        if (isSubMenu) {

...
let _customizeDragX = 0, _customizeDragY = 0;
...
/* ── Drag the preview card around within its pane ──
   Transform-based (not left/top) so it starts exactly where the flex
   layout naturally centers it, with no positioning math needed. */
function _customizeInitDrag() {
    ...
    wrap.addEventListener('pointerdown', e => {
        ...
        baseX = _customizeDragX; baseY = _customizeDragY;
        wrap.setPointerCapture(e.pointerId);
    });
    wrap.addEventListener('pointermove', e => {
        if (!dragging) return;
        _customizeDragX = baseX + (e.clientX - startX);
        _customizeDragY = baseY + (e.clientY - startY);
        wrap.style.transform = `translate(${_customizeDragX}px, ${_customizeDragY}px)`;
    });

...
async function initGame() {
    ...
    // Music transition hook — was a Web Audio crossfade, now a no-op
    // stub (startBgAudio) until the .ogg-based player is wired up.
...
function returnToMenu() {
    ...
    // Music transition hook — no-op stub until .ogg player is wired up.
...
function saveSettings() {
    const s = {
        ...
        uiScale:       _optValue('opt-ui-scale'),
...
function loadSettings() {
    ...
    setValue('opt-ui-scale',   s.uiScale);
    ...
    const scaleLbl = document.getElementById('scale-label'); if (scaleLbl && s.uiScale != null) scaleLbl.textContent = (s.uiScale/10).toFixed(1)+'×';
...
function _applySettingsEffects(s) {
    ...
    // UI scale — clamp to the slider's actual range (0.6x–1.4x) and
    // fall back to 1 for anything non-numeric or out of bounds, so a
    // stray/corrupted value in localStorage can never silently blow the
    // whole game up to some huge size with no setting visibly "changed".
    let uiScaleNum = parseFloat(s.uiScale);
    if (!isFinite(uiScaleNum) || uiScaleNum < 6 || uiScaleNum > 14) uiScaleNum = 10;
    document.documentElement.style.setProperty('--ui-scale', uiScaleNum / 10);

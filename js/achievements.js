...
function trackGameEnd(won) {
    ...
    // Weekly tournament hook — only fires if this game was a tournament match
    if (typeof _wtOnMatchEnd === 'function') _wtOnMatchEnd(won);
    if (won) {
        achStats.wins++;
        achStats.winStreak++;
        ...
        // Speedrun tracking — record fastest win (in seconds)
        if (typeof _battleStartTs !== 'undefined' && _battleStartTs) {
            const durationSec = Math.max(1, Math.round((Date.now() - _battleStartTs) / 1000));
            if (typeof _recordSpeedrunTime === 'function') _recordSpeedrunTime(durationSec);
        }
...
// initialization happens in auth.js via _authStartupCheck)
